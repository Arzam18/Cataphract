...
void start()
{
    ...
    std::println("Cataphract 1.6 by masceron");
    std::fflush(stdout);
}

void process_move(Position& position, const std::string_view move, MoveList& list)
{
    if (move.length() == 4)
    {
...
void set_board(const std::string_view fen)
{
    ...
    {
        if (fen_parse(position, fen.substr(4, moves_pos == std::string::npos ? std::string::npos : moves_pos - 5)) == -
            1)
        {
            return;
        }
