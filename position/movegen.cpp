...
[[nodiscard]] Move* MoveList::end() const
{
    return last;
}

[[nodiscard]] int64_t MoveList::size() const
{
    return last - list;
}

Move MoveList::operator[](const int index) const
{
    return list[index];
}

MoveList::MoveList() : last(list)
{
}
...
void legals(const Position& cr_pos, MoveList& list)
{
    ...
    pseudo_legals<MoveType::all>(cr_pos, list);
...
template void pseudo_legals<MoveType::quiet>(const Position&, MoveList&);
template void pseudo_legals<MoveType::noisy>(const Position&, MoveList&);
